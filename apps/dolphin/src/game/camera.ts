// Spring-damped chase camera. Restraint is the feature: it trails the
// dolphin's heading with a soft catch-up, lifts slightly on a breach so
// the leap reads, and never cuts or snaps.
...
export class ChaseCamera {
  ...
  update(s: SimState, dt: number): void {
    ...
    const target = new THREE.Vector3(
      s.x - dirX * BACK,
      Math.min(s.y + UP + airLift, 6), // camera may poke above water on a breach
      s.z - dirZ * BACK,
    );
