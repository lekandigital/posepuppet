...
S9.1 | Realistic look holds up in motion (flight) | "Convincing, grounded, not photoreal" is an aesthetic judgment no metric captures | V4 tunnel, http://localhost:5176/openworld/?profile=realistic&mode=flight | Fly the fjord ~3 min: over the sun path on the water, along the shaded wall, over the town, up to the snow line | Reads as a believable cold place in motion: sun glitter moves naturally, slopes shade plausibly, painted roofs and birch stands sell the town, haze gives depth; no low-poly tell, no uncanny photoreal reach; controls IDENTICAL to low-poly (same data, same sims) | Same flight.spec chain (profile-blind); consistency spec proves realistic ≡ low-poly geography byte-identically; perf 60 fps headed :2 (275k tris); board realistic-*.png + vision review | An art pass that only works in stills ships unexamined | 5

S9.2 | Realistic walk + row: ground-level material read | Street-level is where vertex-color terrain and painted walls succeed or fail | Same, ?profile=realistic&mode=walk then ?mode=row | Walk the settlement 2 min (watch walls, road surface, shaded grass); row 2 min (watch the water surface up close, the glint, the shore) | Materials read at 1.6 m eye height: walls look painted not plastic, terrain grain breaks flat faces, water up close moves without pattern artifacts; comfort identical to low-poly (same locomotion) | walk.spec/row.spec (profile-blind) green; S8.1 comfort caps enforced in-model regardless of profile | Ground-level material failures unseen from the air | 4

S9.3 | Region-edge honesty in realistic | The bbox-clipped sea read over a ridge is a known honest artifact; judge if it needs a future treatment | Same, ?mode=walk near the region edge (uphill from the settlement, look outward) | Look across the region edge from high ground | The world visibly ENDS at the data boundary (straight sea edge); it should read as "edge of the map", not as a bug — if it grates, file a FUTURES note (horizon skirt) rather than a defect | EVAL_NOTES §V4 O8 records the artifact and the accepted rationale | An unexplained visual would be reported as a bug later | 2

### S10 Fantasy profile: the one-second-clip charm test per mode

S10.1 | The one-second-clip charm test (flight/walk/row) | "Would an unexplained short clip look instantly charming and distinctive?" is the profile's own acceptance question — human-only by definition | V4 tunnel, http://localhost:5176/openworld/?profile=fantasy-game per mode | Record (or just watch) ~15 s of each mode: a banking pass over the town at dusk, a walk down a lane past glowing windows and chimney smoke, a row across the pastel water past a sailboat; find the windmill and the lighthouse from the air | Instant read: a beautifully art-directed miniature place — golden light, peach/lavender/soft-green palette, visible life (clouds, birds, smoke, boats, windmill blades, fireflies, the faint aurora); geography still OBVIOUSLY Ísafjörður; nothing childish, plastic, or clip-art; the magical accents stay restrained | Boards .shots/board/fantasy-game-*.png + vision self-review (EVAL_NOTES §V4 O9, incl. the aurora edge-on fix round); 60 fps headed :2 at 337k tris; consistency spec proves identical geography | Shipping a "fantasy" that is just a palette swap — or worse, a kitsch one — unexamined | 6

S10.2 | Fantasy controls identical + content matrix honest | Cross-profile control identity is felt, not just asserted | Same, switch low-poly ↔ fantasy mid-session via the selector | Fly one lap in low-poly then the same lap in fantasy; confirm the mode list has no dolphin in fantasy and the row coach never offers the dive | Controls indistinguishable between profiles (same sims, same data — any felt difference is a bug); dolphin absent from fantasy exactly as labeled | Battery byte-identical across all three profiles (consistency spec output names them); mode/content matrix spec | A profile that silently changes handling breaks the one-foundation promise | 3
